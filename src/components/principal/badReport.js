  ...
  verifyCOVID = () => {
    ...
    this.state.symptoms.map(symptom => {
      if (symptom == 'Febre') {
        this.state.symptoms.map(symptom => {
          if (
            symptom == 'DordeGarganta' ||
            symptom == 'DificuldadeParaRespirar' ||
            symptom == 'Tosse'
          ) {
            cont_1 = cont_1 + 1;
          }
  ...
  sheradReport = () => {
    if (this.state.contactWithSymptom != null) {
      Alert.alert(
        ...
        [
          {
            text: 'Compartilhar e Reportar',
            onPress: () => {
              if (Platform.OS === 'ios') {
                Linking.openURL(
                  'whatsapp://send?text=Baixe o nosso aplicativo e nos ajude.... https://apps.apple.com/us/app/guardi%C3%B5es-da-sa%C3%BAde/id1450965975?l=pt&ls=1',
                );
                this.verifyLocalization();
              } else {
                Linking.openURL(
                  'whatsapp://send?text=Baixe o nosso aplicativo e nos ajude.... https://play.google.com/store/apps/details?id=com.guardioesapp&hl=pt',
                );
                this.verifyLocalization();
              }
            },
  ...
  render() {
    ...
    const traveled = (
      <View style={styles.viewRowCenter}>
        <View>
          <Text style={styles.commomTextView}>
      ...
      <View style={styles.container}>
        <View style={styles.Top}>
          ...
          <View style={styles.UserInfos}>
            <Text style={styles.UserName}>{this.state.userSelected}</Text>
          </View>
        </View>
        <View
          style={{
          ...
          <Text style={styles.dateText}>{translate('badReport.sickAge')}</Text>
          <DatePicker
            style={{
              width: '94%',
              marginLeft: '3%',
        ...
        <ScrollView style={styles.scroll}>
          ...
          {this.state.contactWithSymptomCheckbox ? (
            <View style={styles.viewRowCenter}>
              <View>
                <Text style={styles.commomTextView}>
                  Selecione o local onde ocorreu o contato
                </Text>
              </View>
              <ModalSelector
                initValueTextStyle={{color: 'black'}}
          ...
          <View style={styles.buttonView}>
            <Button
              title={translate('badReport.checkboxConfirm')}
              color="#348EAC"
              onPress={() => this.verifyCOVID()}
            />
          </View>
        </ScrollView>
        <AwesomeAlert
          ...
          message={
            this.state.progressBarAlert ? null : (
              <Text>
                {translate('badReport.alertMessages.reportSent')}
                {'\n'}
                {translate('badReport.alertMessages.seeADoctor')} {emojis[0]}
                {emojis[0]}
                {emojis[0]}
              </Text>
            )
